...
def suggest_tags(text_sample: str, known_tags: list[str]) -> dict:
    ...
    prompt = (
        "A new document is being uploaded to an enterprise knowledge base with tag-based "
        "access control. Existing tags in use: " + ", ".join(known) + ".\n\n"
        f"Document excerpt:\n{text_sample[:2000]}\n\n"
        "Suggest which tag(s) this document should get. Rules:\n"
        "1. If the content is specific to one department (HR policy, finance figures, "
        "engineering procedures), tag it with ONLY that department's tag — do NOT also "
        "add 'general', even though it affects employees in that department. Adding "
        "'general' makes it visible to EVERY role, which defeats department-specific "
        "access control.\n"
        "2. Only use 'general' for content that is genuinely meant for all employees "
        "regardless of department — company-wide FAQs, holiday calendars, office policies "
        "that apply to literally everyone.\n"
        "3. If none of the existing department tags fit, propose ONE new short lowercase tag.\n\n"
