...
def extract_error_detail(exc) -> str:
    """
    requests' raise_for_status() only gives a generic 'Internal Server Error' message —
    the actual reason FastAPI raised the error lives in the JSON response body's "detail"
    field. This pulls that out when available, falling back to the generic message
    only if the response truly isn't JSON (e.g. the backend is unreachable at all).
    """
    ...
    with st.chat_message("assistant"):
        with st.spinner("Searching documents and generating answer..."):
            try:
                ...
                st.error(f"Error: {extract_error_detail(e)}")
        ...
        if uploaded_file and st.button("Ingest Document"):
            with st.spinner("Processing document (extracting, chunking, embedding, generating sample questions)..."):
                try:
                    ...
                    st.error(f"Ingestion failed: {extract_error_detail(e)}")
                ...
                with col_retag_btn:
                    ...
                    if st.button("Update tags", use_container_width=True):
                        try:
                            ...
                            st.error(f"Retag failed: {extract_error_detail(e)}")
                with col_delete_btn:
                    st.markdown("<div style='height:28px'></div>", unsafe_allow_html=True)
                    if st.button("🗑️ Delete", use_container_width=True):
                        try:
                            ...
                            st.error(f"Delete failed: {extract_error_detail(e)}")
            else:
                st.caption("No documents ingested yet.")
        except Exception as e:
            st.error(f"Could not load documents: {extract_error_detail(e)}")
